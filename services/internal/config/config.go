...
func (c *AppConfig) RateLimiterSettings() RateLimiterSettings { return c.rateLimiterSettings }
...
func (cb *Builder) Build() *AppConfig {
	cfg := &AppConfig{
		...
		corsSettings: CORSSettings{
			corsEnabled:    cb.getenvBool("RATE_LIMIT_ENABLED", true),
...
func (cb *Builder) getenv(key, fallback string) string {
	...
	log.Printf("ENV %s is empty, using fallback %s", key, fallback)
	return fallback
}
...
func (cb *Builder) getenvInt(key string, fallback int) int {
	...
	log.Printf("ENV %s is empty or invalid, using fallback %d", key, fallback)
	return fallback
}
...
func (cb *Builder) getenvBool(key string, fallback bool) bool {
	...
	log.Printf("ENV %s is empty or invalid, using fallback %v", key, fallback)
	return fallback
}
...
func (cb *Builder) getenvList(key string, fallback []string) []string {
	...
	log.Printf("ENV %s is empty, using fallback %v", key, fallback)
	return fallback
}
